...
function TablePresentation({ staredRowIds, onChangeRowStar, sorter, onChangeSorter, rows }) {
    ...
    const renderRows = () => {
        return <div className={'dt-rows'}>{rows.slice(0, showingRowsLength).map(row => {
            const isStared = staredRowIds.includes(row.id);
            return (
                <div key={row.id} className={isStared ? 'stared' : ''} onClick={() => onChangeRowStar(row.id)}>
    ...
    return (
        <div className={'dashboard-table'}>
            {renderTitles()}
            {renderRows()}
            <div className='dt-load-more' onClick={showMore}><img src={expandIcon} alt={'more'} /></div>
        </div>
    );
}
