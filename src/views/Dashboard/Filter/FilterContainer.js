...
const initialFilters = Object.values(availableFilters).reduce((result, item) => (
    {...result, [item]: initialValuesBasedOnTypes[availableFiltersValues[item].type]}), {});
...
function FilterContainer({ filters, setFilters }) {
    ...
    useEffect(() => {
        if(JSON.stringify(urlSearchValues) !== JSON.stringify(prevUrlSearchValues)) {
            setFilters(urlSearchValues);
        }
    }, [urlSearchValues]);

    const onChangeFilters = (newValue) => {
        history.push(`/dashboard?${new URLSearchParams({...(filters), ...newValue}).toString()}`);
    };

    return <FilterPresentation {...{ filters: {...initialFilters, ...(filters)}, onChangeFilters }} />;
}
